...
function getSimpleSections(post = {}) {
  return (post.simpleSections || []).filter((section) => {
    const image = normalizeImage(section.image || section.imageUrl || '', '');
  ...
  return (
    <>
      <style>{`
        ...
        .bp-h1 { max-width: 760px; margin-top: 18px; font: 700 clamp(2.25rem, 4.6vw, 4.85rem)/1.03 "Playfair Display", serif; color: #2f2723; }
      ...
      <main className="bp-page">
        <section className="bp-shell bp-hero">
          ...
          <div className="bp-hero-img">
            <LazyImage src={image} alt={normalized.featuredImage.alt || ''} sizes="(min-width: 900px) 42vw, 100vw" />
          </div>
        </section>
            ...
            {hasSimpleSections ? (
              <>
                ...
                {simpleSections.map((section, index) => {
                  const sectionImage = normalizeImage(section.image || section.imageUrl || '', '');
                ...
                <div>
                  {normalized.images.filter((imageItem) => imageItem.url).map((imageItem) => (
                    <figure key={imageItem.id || imageItem.url}>
                      <img src={imageItem.url} alt={imageItem.alt || ''} width={imageItem.width || undefined} height={imageItem.height || undefined} loading="lazy" />
